...
describe('simulateToEnd', () => {
  test('consome o plano inteiro e termina a partida', () => {
    ...
    // Act
    const { value } = simulateToEnd(match, NO_DICE, ALWAYS, createRng(7))
  ...
  test('é determinístico para o mesmo rng', () => {
    ...
    // Act & Assert
    expect(simulateToEnd(match, NO_DICE, probs, createRng(5))).toEqual(
      simulateToEnd(match, NO_DICE, probs, createRng(5)),
    )
  })
  ...
  test('partida já terminada devolve estado intacto e zero eventos', () => {
    // Arrange
    const match = startMatch(42, NO_DICE)
    const done = simulateToEnd(match, NO_DICE, ALWAYS, createRng(7)).value.state

    // Act
    const again = simulateToEnd(done, NO_DICE, ALWAYS, createRng(8))
