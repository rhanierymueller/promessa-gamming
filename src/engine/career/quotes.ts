...
const fill = (
  template: string,
  values: { craque: string; rival: string; time: string },
): string =>
  template
...
export const matchQuotes = (save: PlayerSave): readonly MatchQuote[] => {
  ...
  const mate = pickSpeaker(squadPlayersFor(myClub, save.careerYear), seed)
  if (mate) {
  ...
  const foe = pickSpeaker(squadPlayersFor(rivalClub, save.careerYear), seed ^ 0x51ed2701)
  if (foe) {
